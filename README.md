...
An original, single-player gothic pixel-art action platformer for the browser. **The Long Night / 1.3** extends every district by 78%, recrafts all five guardians with connected, sculpted silhouettes, stages five bespoke entrances, and expands each district's musical arrangement. The campaign retains the Bestiary's fifteen enemy species and 31 signature techniques, Reawakening's illustrated prologue, survivors, directional air dashes, physical boss transformations, and two endings. Inspired by the atmosphere and deliberate combat of gothic soulslikes; no Bloodborne characters, locations, assets, music, or story are used.
...
| The Hollow Heart | The Heart of Vesper | **The City Unmade** unfurls a veined mantle and releases curved rapture bolts. |

Each guardian receives an 8.4–9.4 second, letterboxed entrance with an omen, physical arrival, delayed name reveal, and synchronized musical cues. The Warden descends on chains; the Widow emerges through opening roots; the Cantor rises from the water with his congregation; the Astronomer materializes as a constellation aligns; the Heart awakens within the city's arteries. Guardians remain hidden before their first reveal. At half health, a protected transformation changes their silhouette, size, identity, and attack sequence—not just their color or speed. Entrances shorten to 4.2 seconds on repeat attempts and can always be skipped. Skipping a transformation still applies its new form. Reduced motion removes arrival displacement and moving cameras while retaining the readable reveal.

### The longer road

Every district now spans **6,800 world pixels instead of 3,820**, with fifteen ordinary encounters, two new landmark areas, elevated routes, and two optional sheltered memories. Climb the upper ledges to recover their stories and one-time echo rewards; both are preserved in the archive. The lower road remains traversable without taking these detours.

| District | New areas |
|---|---|
| Gaslit Ward | Lamplighters' Rooftops / Foundry of Names |
| Weeping Garden | Glasshouse Canopy / Orchard of Sleep |
| Drowned Choir | Flooded Belfries / Ferrier's Aqueduct |
| Astral Spire | Meridian Stair / Gallery of Unseen Stars |
| Hollow Heart | Artery Causeway / Houses Within |

There are **four resting lamps per district**, including one immediately before the guardian. The original two lamp positions are unchanged. Upper routes reconnect with the lower road, and the new lamps divide the extended journey into manageable runs.
...
The expansion suite checks all four lamps in each district, traverses both elevated memory routes using the actual movement simulation, verifies one-time rewards and old-save migration, and checks staged entrance cues and skips. Browser checks also capture all ten extended routes and fifteen arrival stages, and render eight bars of all fifteen exploration/boss/mutation arrangements into real audio buffers to check silence, clipping, distinct output, and district loudness balance.
...
All game scenery, architecture, sprites, animations, weather, and effects are generated by the original Canvas renderer. Articulated hunter sprites have a feathered hat, half-mask, moving coat, lantern, curved sabre, and move-specific poses. The Long Night replaces separately outlined boss plates with continuous curved contours, tapered joints, overlapping cloth, restrained edge lighting, clipped material grain, and clear facial/core focal points. Both forms of every guardian use this treatment. Blender is not required for this native 2D art pipeline.
...
The soundtrack is synthesized locally with Web Audio. Each location has its own melody, answering phrase, sixteen-bar arrangement cycle, chord progression, meter, instrumentation, and accompaniment, plus separate guardian and mutation arrangements. There is no shared blanket string pad: the Ward uses a felt-piano waltz, the Garden interlocking harp patterns, the Choir low organ and long vocal responses, the Spire asymmetric glass figures, and the Heart bowed cello with a paired heartbeat. Mix trims keep transitions balanced. Score changes crossfade; three entrance cues and movement/combat effects are synthesized too. No downloaded music or sound packs are used.

| District | Original score | Lead sound |
|---|---|---|
| Gaslit Ward | Gaslight, in Three | Felt piano |
| Weeping Garden | Seeds for a Tomorrow | Harp |
| Drowned Choir | A Hymn Below the Water | Synthesized choir and organ |
| Astral Spire | The Seventh Unmoving Star | Glass and bells |
| Hollow Heart | Everything We Could Not Keep | Bowed strings and cello |
...
Core files: `src/game.js` (simulation, movement, encounters, cinematic/dialogue states), `src/attacks.js` (signature definitions, warning rules and beam geometry), `src/effects.js` (projectile and hazard artwork), `src/renderer.js` (composition, scenery, camera and prologue plates), `src/art.js` (drawing primitives), `src/actors.js` (character animation and portraits), `src/guardians.js` (sculpted boss artwork), `src/entrance.js` (arrival choreography), `src/world.js` (animated environments and landmarks), `src/audio.js` (score/effects), `src/content.js` (campaign and conversations), `src/input.js` (devices), `src/storage.js` (persistence), and `src/main.js` (UI).
